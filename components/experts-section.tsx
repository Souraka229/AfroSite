        ...
        <div className="grid grid-cols-1 gap-5 sm:grid-cols-2 lg:grid-cols-4">
          {experts.map((expert, index) => (
            <div
              ...
              className="group flex flex-col items-center rounded-xl border border-border bg-white p-7 text-center hover:border-foreground/20 hover:shadow-md transition-all duration-300"
            >
              <div
              ...
              <h3 className="mt-4 text-sm font-bold text-foreground group-hover:text-accent transition-colors">
                {expert.prenom} {expert.nom}
              </h3>
              <p className="mt-1 text-xs font-semibold text-muted uppercase tracking-widest">{expert.role}</p>
              {expert.description && (
              ...
              {expert.competences && expert.competences.length > 0 && (
                <div className="mt-4 flex flex-wrap justify-center gap-1.5">
                  {expert.competences.slice(0, 3).map((c) => (
                    <span key={c} className="inline-block rounded-full bg-foreground/5 px-2.5 py-0.5 text-xs text-foreground/70">
                      {c}
                    </span>
                  ))}
                </div>
